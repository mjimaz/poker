...
public class poker {
    ...
    public static void main(String[] args) {
        ...
        try{
			...
			FileInputStream fstream = new FileInputStream("poker.txt");
			BufferedReader br = new BufferedReader(new InputStreamReader(fstream));
			
			// Open the file to write to			
			FileOutputStream fout = new FileOutputStream("winners.txt");
			BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(fout));
	
			bw.write("Player 1\t\tPlayer 2\t\tWinner\t\t\tReason\n");
			//Read File Line By Line
			while ((hand = br.readLine()) != null)   {
				game.reason="";
	  			
    			bw.write(hand.substring(0,14));
    			bw.write("\t\t");
    			bw.write(hand.substring(15,29));
    			bw.write("\t\t");
		        						...
		        						if(three_kind == 0){
		        							...
		        							if(two_pairs == 0){
		        								...
		        								if(one_pair == 0){        								
		        									...
		        									bw.write(game.final_result(high_card, hand));
		        								}
		        								else{
		        									bw.write(game.final_result(one_pair, hand));
		        								}
		        							}
		        							else{
		        								bw.write(game.final_result(two_pairs, hand));
		        							}
		        							
		        							}
		        						else{
		        							bw.write(game.final_result(three_kind, hand));
		        						}
		        						
		        					}
		        					else{
		        						bw.write(game.final_result(straight, hand));
		        					}
		        				}
		        				else{
		        					bw.write(game.final_result(flush, hand));	
		        				}
		        			}
		        			else{
		        				bw.write(game.final_result(full_house, hand));
		        			}
		        		}
		        		else{
		        			bw.write(game.final_result(four_kind, hand));
		        		}
		        	}
		        	else{//there is a straight flush
		        		bw.write(game.final_result(straight_flush, hand));
		        	}
		        	
		        }else{//there is a royal flush
		        	bw.write(game.final_result(royal_flush, hand));        	
		        }
        
		        bw.write("\t\t");
		        bw.write((game.reason).trim());
		        bw.write("\n");
			}
